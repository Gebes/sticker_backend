...
// QueryStickers chains the current query on the "stickers" edge.
func (uq *UserQuery) QueryStickers() *StickerQuery {
	query := &StickerQuery{config: uq.config}
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withStickers: uq.withStickers.Clone(),
...
// WithStickers tells the query-builder to eager-load the nodes that are connected to
// the "stickers" edge. The optional arguments are used to configure the query builder of the edge.
func (uq *UserQuery) WithStickers(opts ...func(*StickerQuery)) *UserQuery {
	query := &StickerQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
	uq.withStickers = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	...
	if query := uq.withStickers; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[string]*User)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.Stickers = []*Sticker{}
		}
		query.withFKs = true
		query.Where(predicate.Sticker(func(s *sql.Selector) {
			s.Where(sql.InValues(user.StickersColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.sticker_owner
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "sticker_owner" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "sticker_owner" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.Stickers = append(node.Edges.Stickers, n)
		}
	}
